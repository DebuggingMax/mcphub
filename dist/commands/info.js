...
import { info, formatStats } from '../utils/output.js';
...
export async function infoCommand(name, options) {
  ...
  ${chalk.gray('Categories:')}   ${server.categories.map(c => getCategoryDisplay(c)).join(', ')}
...
`);
  ...
  "${server.name}": {
    ...
    "args": ${JSON.stringify(server.config.args || [])}${server.config.env

...
export async function listCommand(options) {
    ...
    if (options.enabled !== undefined) {
        servers = Object.fromEntries(Object.entries(servers).filter(([_, s]) => s.enabled === options.enabled));
    }
    if (options.json) {
...
export async function enableCommand(name, options) {
    const config = getConfig();
...
export async function disableCommand(name, options) {
    const config = getConfig();
    const server = config.getInstalledServer(name);
    if (!server) {
        console.error(`Server "${name}" is not installed`);
